...
export default function Register() {
  ...
  useEffect(() => {
    ...
    window.google.accounts.id.renderButton(
      ...
      { type: "standard", shape: "pill", theme: "outline", size: "large", text: "continue_with", width: "100%" },
    );
  }, [isAuthenticated]);
  ...
  async function handleGoogleCredential(googleResponse) {
    ...
    try {
      const res = await googleLogin(googleResponse.credential, roleRef.current);
      const { token, user } = res.data;
      loginAction(token, user);
      navigate(ROLE_LINKS[user.role] || "/", { replace: true });
    } catch (err) {
      const msg = err.response?.data?.error || "";
      if (msg.includes("already belongs to an existing account")) {
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            ...
            {googleEnabled && (
              <>
                ...
                <div id="google-register-button" className="w-full flex justify-center min-h-[44px]" />
              </>
            )}

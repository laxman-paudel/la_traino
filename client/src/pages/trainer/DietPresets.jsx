...
import { getGlobalDietPresets, importGlobalDietPreset } from "../../api/trainer";
...
export default function DietPresets() {
  const { addToast } = useToast();
  const [presets, setPresets] = useState([]);
  const [loading, setLoading] = useState(true);
  const [modal, setModal] = useState(null);
  const [previewPreset, setPreviewPreset] = useState(null);
  const [form, setForm] = useState({ name: "", description: "", meals: [] });
  ...
  useEffect(() => {
    getDietPresets()
      ...
      .catch(() => addToast("Failed to load diet presets", "error"))
      .finally(() => setLoading(false));
  }, []);

  function openAdd() {
    setForm({ name: "", description: "", meals: [{ time: "", items: "" }] });
    setModal("add");
  }
  ...
  async function handleSave(e) {
    ...
    const meals = form.meals
      .filter((m) => m.time.trim())
      .map((m) => ({
        time: m.time.trim(),
        items: m.items.split(",").map((i) => i.trim()).filter(Boolean),
      }));
    if (meals.length === 0) { addToast("At least one meal with items is required", "error"); setSaving(false); return; }
    const payload = {
      name: form.name.trim(),
    ...
    try {
      if (modal === "add") {
        const res = await createDietPreset(payload);
        setPresets((prev) => [res.data, ...prev]);
        addToast("Diet preset created", "success");
      } else {
        const res = await updateDietPreset(modal.id, payload);
        setPresets((prev) => prev.map((p) => (p.id === modal.id ? res.data : p)));
        addToast("Diet preset updated", "success");
      }
      setModal(null);
    } catch (err) {
  ...
  async function handleDuplicate(id) {
    try {
      const res = await duplicateDietPreset(id);
      setPresets((prev) => [res.data, ...prev]);
      addToast("Diet preset duplicated", "success");
    } catch (err) {
  ...
  async function handleDelete(id) {
    setDeleting(true);
    try {
      await deleteDietPreset(id);
      setPresets((prev) => prev.filter((p) => p.id !== id));
      setDeleteTarget(null);
      addToast("Diet preset deleted", "success");
    } catch (err) {
  ...
  function addMeal() {
    setForm((f) => ({ ...f, meals: [...f.meals, { time: "", items: "" }] }));
  }
  ...
  function updateMeal(index, field, value) {
    setForm((f) => ({
      ...f,
      meals: f.meals.map((m, i) => (i === index ? { ...m, [field]: value } : m)),
    }));
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Diet Presets"
        subtitle="Create and manage reusable diet plan templates"
        actions={
      ...
      {presets.length === 0 ? (
        <EmptyState
          icon="clipboard"
          title="No Diet Presets"
          message="Create your first reusable diet template to speed up assignments."
          action={
            <button
              type="button"
              onClick={openAdd}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {presets.map((preset) => (
            ...
            >
              ...
              <p className="text-xs text-gray-400">{Array.isArray(preset.meals) ? preset.meals.length : 0} meals</p>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={() => setPreviewPreset(null)}>
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="space-y-3">
              {Array.isArray(previewPreset.meals) && previewPreset.meals.map((meal, i) => (
                <div key={i} className="bg-gray-50 rounded-xl p-4">
                  <p className="text-sm font-semibold text-gray-900 mb-1">{meal.time}</p>
                  {Array.isArray(meal.items) && (
                    <div className="flex flex-wrap gap-1.5">
                      {meal.items.map((item, j) => (
                        <span key={j} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-50 text-indigo-600">{item}</span>
                      ))}
                    </div>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg max-h-[85vh] overflow-y-auto" role="dialog" aria-modal="true">
            <h2 className="text-xl font-bold text-gray-900 mb-1">
              {modal === "add" ? "New Diet Preset" : "Edit Diet Preset"}
            </h2>
            <p className="text-sm text-gray-500 mb-5">
              {modal === "add" ? "Create a reusable diet template" : "Update your diet template"}
            </p>
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button type="button" onClick={addMeal} className="text-xs text-indigo-600 hover:text-indigo-700 font-medium">+ Add Meal</button>
                </div>
                <div className="space-y-3">
                  {form.meals.map((meal, i) => (
                    <div key={i} className="border border-gray-200 rounded-xl p-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {form.meals.length > 1 && (
                          <button type="button" onClick={() => removeMeal(i)} className="text-xs text-red-500 hover:text-red-700 font-medium">Remove</button>
                        )}
                      </div>
                      <div className="space-y-2">
                        <input type="text" placeholder="Meal time (e.g. Breakfast, Lunch)" value={meal.time}
                          onChange={(e) => updateMeal(i, "time", e.target.value)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm" />
                        <input type="text" placeholder="Food items (comma separated)" value={meal.items}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto" role="dialog" aria-modal="true">
            ...
            {globalPresetsLoading ? (
              ...
              <p className="text-sm text-gray-500 text-center py-4">No global diet templates available.</p>
            ) : (
              <div className="space-y-2">
                  ...
                  <div key={gp.id} className="border border-gray-200 rounded-xl p-4 flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        ...
                        <span className="text-xs px-2 py-0.5 bg-gray-100 text-gray-500 rounded-full">{Array.isArray(gp.meals) ? gp.meals.length : 0} meals</span>
                      </div>
                    </div>
                    <button
                      type="button"
                      ...
                      onClick={async () => {
                        ...
                        try {
                          await importGlobalDietPreset(gp.id);
                          addToast(`Imported "${gp.name}"`, "success");
                          getDietPresets().then((res) => setPresets(res.data)).catch(() => {});
                        } catch (err) {
                      ...
                      className="shrink-0 px-3 py-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed text-xs font-semibold transition"
                    >
                      {importingId === gp.id ? (

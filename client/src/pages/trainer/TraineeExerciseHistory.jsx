...
import { getTrainerExerciseHistory } from "../../api/exerciseHistory";
...
export default function TraineeExerciseHistory() {
          ...
          <div className="flex gap-1.5">
            {PERIODS.map((p) => (
              <button key={p.value} type="button" onClick={() => setPeriod(p.value)}
                className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition ${period === p.value ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
                {p.label}
              </button>
            ))}
          </div>

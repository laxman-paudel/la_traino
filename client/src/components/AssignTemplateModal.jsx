...
export default function AssignTemplateModal({ template, isOpen, onClose, onComplete }) {
  ...
  function handleNext() {
    if (step === STEPS.SELECT_TRAINEES && selectedIds.length > 0) {
      setStep(STEPS.SELECT_DAY);
    } else if (step === STEPS.SELECT_DAY && day) {
      setStep(STEPS.PREVIEW);
    }
  }

  function handleBack() {
    if (step === STEPS.SELECT_DAY) setStep(STEPS.SELECT_TRAINEES);
    else if (step === STEPS.PREVIEW) setStep(STEPS.SELECT_DAY);
  }
  ...
  async function handleConfirm() {
    ...
    setSubmitting(true);
    try {
      const res = await assignTemplate(template.id, {
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {step === STEPS.SELECT_TRAINEES && "Choose trainees to assign this template to"}
              {step === STEPS.SELECT_DAY && `Selected ${selectedIds.length} trainee${selectedIds.length !== 1 ? "s" : ""}`}
              {step === STEPS.PREVIEW && "Review before assigning"}
            </p>
          </div>
        ...
        <div className="p-5">
          ...
          {step !== STEPS.RESULT && (
            <div className="flex gap-3 pt-4 border-t border-gray-100 mt-4">
              ...
              <button type="button"
                onClick={step === STEPS.PREVIEW ? handleConfirm : handleNext}
                disabled={
                  (step === STEPS.SELECT_TRAINEES && selectedIds.length === 0) ||
                  (step === STEPS.SELECT_DAY && !day) ||
                  submitting
                }
                ...
                {submitting ? (
                  ...
                  `Assign to ${selectedIds.length} Trainee${selectedIds.length !== 1 ? "s" : ""}`
                ) : "Next"}
              </button>
            </div>
          )}
          ...
          {step === STEPS.RESULT && (
            <button type="button" onClick={onClose}
              className="w-full py-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition font-semibold text-sm mt-4">
              Done
            </button>
          )}
        </div>
